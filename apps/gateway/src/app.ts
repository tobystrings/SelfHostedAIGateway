...
import { builtInAdapters, createAdapter } from "./adapters/index.js";
import { ProviderRegistry } from "./services/provider-registry.js";
import { ModelRegistry } from "./services/model-registry.js";
import { RoutingEngine } from "./services/router.js";
import { BudgetService } from "./services/budget.js";
import { GatewayService } from "./services/gateway.js";
import { AuthService } from "./services/auth.js";
import { UsageService } from "./services/usage.js";
import { ScopedRateLimitService } from "./services/rate-limit.js";
...
export async function buildApp(o: BuildOptions = {}) {
  ...
  await app.register(rateLimit, {
    ...
    keyGenerator: (req) => {
      ...
      return h?.startsWith("Bearer ")
        ? `key:${apiKeyHash(h.slice(7)).toString("hex")}`
        : `ip:${req.ip}`;
    },
  });
  await app.register(swagger, {
    openapi: { info: { title: "Self-Hosted AI Gateway", version: "0.1.0" } },
  });
  await app.register(swaggerUi, { routePrefix: "/docs" });
  const providers = new ProviderRegistry(builtInAdapters(config)),
    models = new ModelRegistry(),
    auth = new AuthService(db),
    audit = new AuditService(db),
    usage = new UsageService(db),
    budget = new BudgetService(db),
    limits = new ScopedRateLimitService(db),
    router = new RoutingEngine(models, providers),
  ...
  if (!o.skipBootstrap) {
    for (const [slug, kind, name, base] of [
      ...
      ["ollama", "ollama", "Ollama", config.OLLAMA_BASE_URL],
    ])
      await db.query(
        "INSERT INTO providers(slug,kind,display_name,base_url) VALUES($1,$2,$3,$4) ON CONFLICT(slug) DO NOTHING",
        [slug, kind, name, base],
      );
    await db.query(
      "INSERT INTO budgets(name,subject_type,monthly_spend_limit_usd) VALUES('default-global-monthly-spend','global',$1) ON CONFLICT(name) DO NOTHING",
      [config.DEFAULT_MONTHLY_SPEND_USD],
    );
  }
  try {
    ...
    models.setMany(
      m.rows.map((x: any) => ({
        provider: x.provider_slug,
        id: x.upstream_id,
        displayName: x.display_name,
        enabled: x.enabled,
        capabilities: {
          ...x.capabilities,
          contextWindow: x.context_window ?? x.capabilities?.contextWindow,
          maxOutputTokens:
            x.max_output_tokens ?? x.capabilities?.maxOutputTokens,
        },
        pricing:
          x.input_per_million_usd == null
            ? undefined
            : {
                inputPerMillionUsd: Number(x.input_per_million_usd),
                outputPerMillionUsd: Number(x.output_per_million_usd),
                cachedInputPerMillionUsd:
                  x.cached_input_per_million_usd == null
                    ? undefined
        ...
        metadata: {
          ...x.metadata,
          alias: x.alias,
          routingPriority: x.routing_priority,
        },
      })),
    );
  ...
  (app as any).decorate("requireApiKey", async (req: any) => {
    if (o.testApiIdentity) {
      req.apiIdentity = o.testApiIdentity;
      return;
    }
    const h = req.headers.authorization;
    ...
    req.apiIdentity = await auth.authenticateApiKey(h.slice(7), req.ip);
    if (!req.apiIdentity.scopes?.includes("gateway:invoke")) {
      throw new GatewayError({
        code: "insufficient_scope",
        message: "API key lacks the gateway:invoke scope",
        type: "auth",
        retryable: false,
        status: 403,
      });
  ...
  (app as any).decorate("requireAdmin", async (req: any) => {
    ...
    const c = await verifySession(token, config.SESSION_SECRET);
    if (
      !c.roles.some((r: string) => ["admin", "operator", "viewer"].includes(r))
    )
      throw new GatewayError({
    ...
    if (
      ...
      !req.url.startsWith("/api/admin/playground/")
    )
      throw new GatewayError({
    ...
    if (
      ...
      req.headers["x-csrf-token"] !== c.csrf
    )
      throw new GatewayError({
        code: "csrf_failed",
  ...
  app.addHook(
    "onRequest",
    async (req: any) => (req.metricStart = performance.now()),
  );
  ...
  app.get("/ready", async (_q, r) => {
    ...
    return r
      .code(ok ? 200 : 503)
      .send({ status: ok ? "ready" : "not_ready", database: ok });
  });
  app.get("/metrics", async (_q, r) => {
    r.type(registry.contentType);
    return registry.metrics();
  });
  app.setErrorHandler((error, request, reply) => {
    const gatewayError = normalizeUnknownError(error);
    if (gatewayError.shape.status >= 500) {
      request.log.error({ err: redact(error) }, "request failed");
    }
    reply.code(gatewayError.shape.status).send({
      error: {
        message: gatewayError.shape.message,
        type: gatewayError.shape.type,
        code: gatewayError.shape.code,
      },
      gateway: {
        retryable: gatewayError.shape.retryable,
        provider: gatewayError.shape.provider,
      },
    });
  });
  ...
  const here = path.dirname(fileURLToPath(import.meta.url)),
    admin = path.resolve(here, "../../admin/dist");
  if (fs.existsSync(admin)) {
    ...
    app.get("/admin/*", async (_q, r) => r.sendFile("index.html"));
  }
