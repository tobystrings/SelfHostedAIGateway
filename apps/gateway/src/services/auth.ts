...
export class AuthService {
  ...
  async bootstrapAdmin(email: string, password: string) {
    const count = await this.db.query<{ n: string }>('SELECT count(*)::text n FROM users');
  ...
  async createApiKey(name: string, userId?: string) {
    ...
    const result = await this.db.query<any>(
      'INSERT INTO client_api_keys(user_id,name,key_prefix,key_hash) VALUES($1,$2,$3,$4) RETURNING id,name,scopes,created_at',
      [userId ?? null, name, key.slice(0, 10), hash],
    );
